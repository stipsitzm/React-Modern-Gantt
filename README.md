...
| `localeText`            | `GanttLocaleText`      | `{}`                 | Legacy UI overrides for title/resources/today/view mode labels; prefer `labels` for visible Gantt UI text |
| `labels`                | `Partial<GanttLabels>` | `{}`                 | Consumer-provided labels for `today` and the view mode switcher (`minute`…`year`)                         |
...
You can also localize the visible Gantt UI labels directly from your consuming app (recommended):
...
If you want to override only the title or resource label for a specific screen, you can still pass `title` or `headerLabel` directly. The older `localeText` prop remains supported for backward compatibility, but `labels` is now the preferred API for externally provided UI text.
